  ...
  try {
    ...
    if (!bookDetailsByID) {
      return res.status(404).json({
        success: false,
        message: "Book by this ID doesn't exists! Please try another ID.",
      });
    }
...
const updateSingleBook = async (req, res) => {
  try {
    ...
    if (!updatedBook) {
      res.status(404).json({
        success: false,
        message: "Book doesn't exits of the specified ID.",
      });
    }
...
const deleteBook = async (req, res) => {
  try {
    ...
    if (!deleteBook) {
      res.status(404).json({
        success: false,
        message: "Book is not found of the specified ID.",
      });
    }
